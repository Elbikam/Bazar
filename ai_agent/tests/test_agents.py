...
async def test_quantity_sold_it():
    query = "what is total quantity sold it for month June 2025?"
...
async def test_20_80():
    # q="i want to find the items that contribute to 80% of the total revenue?"
    query = "i want to find the items that contribute to 80% of the total revenue?"
    response = await call_root_agent(query)
    print(f"response:{response}")
